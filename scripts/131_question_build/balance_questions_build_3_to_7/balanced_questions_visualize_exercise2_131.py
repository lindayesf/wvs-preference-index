"""Exercise 2 visualization for the balanced-questions 131-question build.

Reads current Exercise 2 outputs from results/index_131/balanced_questions_exercise2/:
- weighted_variance_sweep_index_*_ym.csv
- weighted_variance_sweep_index_*_y.csv

Writes figures to results/index_131/balanced_questions_exercise2/figures/.
"""
...
EX2_DIR = RESULTS_DIR / "balanced_questions_exercise2"
...
def plot_reevaluation_grid(group_name: str, suffix: str, title: str) -> None:
    ...
    fig.suptitle(
        f"Sensitivity of GDP-weighted variance to weighting year: {title}\n"
        f"Balanced panel of questions present in WVS waves 3 to 7",
        fontsize=13,
        y=0.99,
    )
...
def main() -> None:
    print("Generating balanced-questions Exercise 2 re-evaluation sensitivity charts...")
    for group_name, cfg in REEVAL_GROUPS.items():
        plot_reevaluation_grid(group_name, cfg["suffix"], cfg["title"])
    print(f"\nAll balanced-questions Exercise 2 figures saved to {FIG_DIR}/")
