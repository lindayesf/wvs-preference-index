...
def load_wvs_data(path: Path, variables: list[str]) -> pd.DataFrame:
    requested = sorted(set(variables) | set(CATEGORICAL_SOURCE_VARS_131))
...
def build_domain_subset_catalog(subset_questions: pd.DataFrame) -> pd.DataFrame: # this creates the correct subset catalog that combines the ordinal variables and the indicator variables, and then adds the domain flags for each variable.
    ...
    ordinal_catalog = subset_questions[
        ~subset_questions["variable"].isin(CATEGORICAL_SOURCE_VARS_131)
...
def main():
    ...
    if len(ym_questions) != 127:
        raise RuntimeError(f"Pause: expected 127 questions for Re-evaluation in {{y,m}}, got {len(ym_questions)}.")
    if len(y_questions) != 86:
        raise RuntimeError(f"Pause: expected 86 questions for Re-evaluation == y, got {len(y_questions)}.")

    ordinal_vars = [v for v in keep131["variable"].tolist()
                     if v not in CATEGORICAL_SOURCE_VARS_131]
    ym_ordinal_vars = [v for v in ym_questions["variable"].tolist()
                       if v not in CATEGORICAL_SOURCE_VARS_131]
    y_ordinal_vars = [v for v in y_questions["variable"].tolist()
                      if v not in CATEGORICAL_SOURCE_VARS_131]

    print(f"  {len(keep131)} questions kept (Keep>=2 after exclusions), {len(ordinal_vars)} "
          f"ordinal + {len(CATEGORICAL_SOURCE_VARS_131)} categorical source vars")
    ...
    all_needed_vars = sorted(
        ...
        | set(ym_ordinal_vars)
        | set(y_ordinal_vars)
        | set(CATEGORICAL_SOURCE_VARS_131)
    )
    df = load_wvs_data(WVS_FILE, all_needed_vars)
    ...
    print("Building categorical -> indicator variables (14 total)...")
    ...
    catalog_ym = build_domain_subset_catalog(ym_questions)
    catalog_y = build_domain_subset_catalog(y_questions)
